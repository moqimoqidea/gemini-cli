...
export class GoogleCredentialProvider implements OAuthClientProvider {
  ...
  constructor(private readonly config?: MCPServerConfig) {
    ...
    if (CLOUD_RUN_HOST_REGEX.test(hostname)) {
      throw new Error(
        `GoogleCredentialProvider no longer issues ID tokens. Configure a custom auth provider module for Cloud Run host ${url}.`,
      );
    }
